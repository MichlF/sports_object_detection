...
def find_contours_harris(
    ...
    corners = cv2.cornerHarris(src=image, blockSize=block_size, ksize=k_size, k=k)
    ...
    _, thresh = cv2.threshold(corners_norm_scaled, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
...
@timer(enabled=TIME_FUNCTIONS)
def get_corners_pts(corner_pts: list, output_res: tuple[int, int] = None,
    show_intermediate_steps: bool = False,):
    ...
    two_top_pts = np.array(
        [
            center_array[center_array[:, 1].argsort()[0]],  # lowest Y val # top
            center_array[center_array[:, 1].argsort()[1]],  # second lowest Y val # top
        ]
    )
    two_bottom_pts = np.array(
        [
            center_array[center_array[:, 0].argsort()[0]],  # lowest X val # left
            center_array[center_array[:, 0].argsort()[-1]],  # highest X val # right
        ]
    )
    two_top_pts_sorted = two_top_pts[two_top_pts[:, 0].argsort()]
    two_bottom_pts_sorted = two_bottom_pts[(-two_bottom_pts[:, 0]).argsort()]
    corners_list = np.concatenate([two_top_pts_sorted, two_bottom_pts_sorted], axis=0)
    if output_res is not None:
        image = np.empty(output_res)
        for point in corners_list:
            cv2.circle(image, center=point, radius=3, color=(255, 255, 255), thickness=3)
        if show_intermediate_steps:
